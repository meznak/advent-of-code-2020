...
def count_neighbors_4(cubes: list, location: tuple) -> int:
    '''Count a cube's active neighbors'''
    count = 0

    for w in range(location[3] - 1, location[3] + 2):
        for z in range(location[2] - 1, location[2] + 2):
            for y in range(location[1] - 1, location[1] + 2):
                for x in range(location[0] - 1, location[0] + 2):
                    if (x, y, z, w) == location or (x, y, z, w) not in cubes:
                        continue
                    if cubes[(x, y, z, w)]:
                        count += 1
    return count
...
def solve_1(dataset: list) -> int:
    ...
    # Set known boundaries of the grid, exclusive of the upper
    min_x = 0
    max_x = len(dataset[0])
    min_y = 0
    max_y = len(dataset)
        ...
        for w in range(min_w - 1, max_w + 2):
            for z in range(min_z - 1, max_z + 2):
                for y in range(min_y - 1, max_y + 2):
                    for x in range(min_x - 1, max_x + 2):
                        cube = (x, y, z, w)
                        neighbors = count_neighbors_4(cubes, cube)
